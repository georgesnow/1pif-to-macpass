...
module.exports = function endHandler() {
  ...
  fs.readFile(path.join(__dirname, '..', 'entry.ejs'), function(err, tpl) {
    ...
    _.forEach(self.entries, function(entry, index) {
        binding = {
        uuid: new Buffer(index.toString()).toString('base64'),
      ...
      switch (entry.typeName) {
        case 'webforms.WebForm':
          userField        = _.find(entry.secureContents.fields, function(f) { return f.designation === 'username'; });
          pwField          = _.find(entry.secureContents.fields, function(f) { return f.designation === 'password'; });
      ...
      self.xml('Group').append(entryXML);
    });
